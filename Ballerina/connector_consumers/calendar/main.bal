...
isolated function delete_calendar() returns error?{
    ...
    error? res = calendarClient->deleteCalendar("primary");
    if (res is error) {
        log:printError(res.message());
    } else {
...
isolated  function get_calendar() returns error? {
    ...
    calendar:CalendarResource|error res = calendarClient->getCalendar("primary");
    if (res is calendar:CalendarResource) {
        log:printInfo(res.id);
    } else {
        log:printError(res.message());
...
isolated  function delete_event() returns error? {
    ...
    error? res = calendarClient->deleteEvent("primary", eventId);
    if (res is error) {
        log:printError(res.message());
    } else {
...
isolated  function get_event() returns error? {
    ...
    calendar:Event|error res = calendarClient->getEvent("primary", eventId);
    if (res is calendar:Event) {
        log:printInfo(res.id);
    } else {
        log:printError(res.message());
...
isolated  function quick_add_events() returns error? {
    ...
    calendar:Event|error res = calendarClient->quickAddEvent("primary", "Sample Event");
    if (res is calendar:Event) {
        log:printInfo(res.id);
    } else {
        log:printError(res.message());
...
isolated function update_event() returns error? {
    ...
    calendar:Event|error res = calendarClient->updateEvent("primary", eventId, event);
    if (res is calendar:Event) {
        log:printInfo(res.id);
    } else {
        log:printError(res.message());
